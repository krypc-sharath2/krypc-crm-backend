const MarketSchema = require("../models/MarketResearchModel");
...
exports.marketList = [
	function (req, res) {
        console.log(req)
		try {
			MarketSchema.find().then((companies)=>{
 ...
 * @returns {Object}
 */
exports.getMarketById = [
	function (req, res) {
		if(!mongoose.Types.ObjectId.isValid(req.params.id)){
			return apiResponse.successResponseWithData(res, "Operation success", {});
		}
		try {
			MarketSchema.findOne({_id: req.params.id}).then((company)=>{
				if(company !== null){
					let companyData = new MarketData(company);
...
exports.addMarket = [
	...
	body("account_name", "Company Name must not be empty.").isLength({ min: 1 }).trim().custom((value,{req}) => {
		return MarketSchema.findOne({account_name : value}).then(company => {
			if (company) {
		...
		try {
			...
			if (!errors.isEmpty()) {
				...
				if(!mongoose.Types.ObjectId.isValid(req.params.id)){
					...
					MarketSchema.findById(req.params.id, function (err, foundCompany) {
						if(foundCompany === null){
							...
							if(foundCompany.user.toString() !== req.user._id){
								...
								//update company.
								MarketSchema.findByIdAndUpdate(req.params.id, company, {},function (err) {
									if (err) { 
										return apiResponse.ErrorResponse(res, err); 
									}else{
										let companyData = new MarketData(company);
			...
			MarketSchema.findById(req.params.id, function (err, foundCompany) {
				if(foundCompany === null){
					...
					if(foundCompany.user.toString() !== req.user._id){
						...
						//delete company.
						MarketSchema.findByIdAndRemove(req.params.id,function (err) {
							if (err) { 
								return apiResponse.ErrorResponse(res, err); 
							}else{
